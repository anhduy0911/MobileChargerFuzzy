...
def prepare_data():
    topo_data = pd.read_csv('data/400node.csv', header=None)
    print(topo_data.head())
    data = topo_data[6].to_numpy()
    import random
    target = random.sample(range(data.shape[0]), k=250)

    topo_string = ','.join([x for x in data])
    target_string = ', '.join([str(x) for x in target])

    old_dat = pd.read_csv('data/formal_data_5.csv', header=0)
    ...
    new_data.to_csv('data/formal_data_5_400.csv')
